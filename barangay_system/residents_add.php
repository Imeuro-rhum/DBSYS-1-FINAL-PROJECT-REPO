...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO barangay_residents (household_id, first_name, last_name, birth_date, gender, contact_number) 
                               VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssss", $household_id, $first_name, $last_name, $birth_date, $gender, $contact_number);
        
        if ($stmt->execute()) {
            $message = showMessage('Resident added successfully!', 'success');
            $_POST = [];
        } else {
            ...
            <div class="col-md-8 mx-auto">
                <div class="card">
                    <div class="card-header">
                        <h4><i class="bi bi-person-plus"></i> Add New Resident</h4>
                    </div>
                    <div class="card-body">
                        <?php echo $message; ?>
                        
                        <form method="POST" action="residents_add.php" onsubmit="return validateResidentForm()">
                            <div class="mb-3">
